...
var ckrphnba = "ws://Atlantis.local:4567";
var ckrislocal1 = "://10.0.0.103:4567";
var ckrislocal2 = "Atlantis.local:4567";
...
function establishckrp(useBonjour) {
    ...
    function displayLagOverlay() {
        if (!document.getElementById(WARNING_OVERLAY_ID) && !document.getElementById(WARNING_LAG_OVERLAY_ID) && lagBannerCanShow === true) {
            ...
            e.innerHTML = '<p style="margin: 0 15px; color:white; font-size:20px; font-weight: medium; line-height:1.4em; font-family:sans-serif; text-align:center;">The connection to CodeKit is unstable.</p><p style="margin: 10px 20px; color:white; font-size:11px; line-height:1.2em; font-family:sans-serif; text-align:center;">This page has not had contact with CodeKit for 15+ seconds. This can be caused by changing networks, putting your Mac to sleep, or just a laggy LAN (e.g. public Wifi). Auto-refreshing may be slow or unreliable. Refresh manually to reconnect to CodeKit or, if the connection is still working, tap this banner to hide it.</p>';
            if ( document.body.firstChild ) {
                document.body.insertBefore(e, document.body.firstChild);
            }
            else {
            ...
            heartbeatInterval = setInterval(function () {
                try {
                    if ( ++missedHeartbeats > 3 ) {
                        displayLagOverlay();
                        console.warn("CodeKit has not sent a heartbeat message in 15+ seconds. The connection to CodeKit MAY have failed, but it is also possible that your network is simply very laggy. Auto-refreshing may be slow or unreliable.")
                    }
                } catch (e) {
    ...
    var socket = new WebSocket(window.location.href.replace('http://', 'ws://'), ["ckrp"]);
    setTimeout(function () {
    ...
    socket.onerror = function (e) {
        if ( u === LOCALHOST_WS_URL && haveTriedLocalhostForSocket === false ) {
            ...
            console.warn("Error connecting to CodeKit's refresh server: " + e.message)
        }
    };
